...
class HybridForceMotion : public TorqueController {
 public:
  ...
  static const Eigen::Matrix<double, 6, 1> kDefaultForce;

  HybridForceMotion(const Vector7d &stiffness = kDefaultStiffness,
  ...
  void setControl(const Vector7d &position, const Eigen::Matrix<double, 6, 1>& force, const Vector7d &velocity = kDefaultDqd);
 ...
 private:
  Vector7d K_p_, K_d_, q_d_, dq_d_, K_p_target_, K_d_target_, q_d_target_, dq_d_target_,tau_ext_init_;
